...
while True:
    try:
        ...
        shop = browser.find_element(
            By.XPATH, '//*[@id="cashier-button"]/span[2]')
        shop.click()
        time.sleep(10)

        for i in range(1, 8):
            bonus = browser.find_element(
                By.XPATH, f'//*[@id="daily-bonus-{i}"]')
            bonus.click()
            time.sleep(5)
        ...
        smtpserver.ehlo()
        smtpserver.login(email, app_password)

        from_address = email
        to_address = email
        email_text = f"{err}"
        smtpserver.sendmail(from_address, to_address, email_text)
